...
当前配置文件的版本是 `21.5.4`，兼容的 macOS 系统版本如下表所示：
...
- 引导程序为 OpenCore 0.6.9，支持引导 macOS 和 Windows；若需要引导其他操作系统需自行添加。
